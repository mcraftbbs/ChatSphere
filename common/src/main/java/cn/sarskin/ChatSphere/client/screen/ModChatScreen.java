...
public class ModChatScreen extends Screen {
    ...
    private static final ResourceLocation SETTINGS_ICON = new ResourceLocation(
            ModInfo.MODID, "textures/gui/settings_gear.png");
    private static final ResourceLocation EMOJI_ICON = new ResourceLocation(
            ModInfo.MODID, "textures/gui/emoji.png");
    private static final ResourceLocation QUICK_PHRASES_ICON = new ResourceLocation(
            ModInfo.MODID, "textures/gui/quick_phrases.png");
    private static final ResourceLocation SEARCH_ICON = new ResourceLocation(
            ModInfo.MODID, "textures/gui/search.png");
    private static final ResourceLocation JOIN_CHANNEL_ICON = new ResourceLocation(
            ModInfo.MODID, "textures/gui/join_channel.png");
    private static final Pattern ITEM_REF_PATTERN = Pattern.compile("\\[\\d+\\]");
    private static final ResourceLocation CREATE_CHANNEL_ICON = new ResourceLocation(
            ModInfo.MODID, "textures/gui/create_channel.png");
    private static final ResourceLocation BLOCK_ICON = new ResourceLocation(
            ModInfo.MODID, "textures/gui/block.png");
    private static final ResourceLocation ITEM_ICON = new ResourceLocation(
            ModInfo.MODID, "textures/gui/item_chest.png");
    private static final ResourceLocation VOICE_ICON = new ResourceLocation(
    ...
    protected void init() {
        ...
        // Restore the last selected conversation (persisted on close).
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (button == 0) {
            ...
            // Emoji autocomplete click (not in the command console)
            if (!COMMAND_CONVERSATION_ID.equals(currentConversation)
        ...
        // Right-click avatar: context menu (@ player)
                    ...
                    switch (item.kind) {
                        case RK_HOME -> {
                            ...
                            if (sidebarPrivateMode) {
                                ...
                                // Leaving the console page: select the first private conversation
                                // so the console no longer stays highlighted.
                                if (COMMAND_CONVERSATION_ID.equals(currentConversation)) {
    ...
    public void removed() {
        super.removed();
        // The screen may be replaced by another screen (setScreen), which never calls
        // onClose(); persist the state here as well, without triggering setScreen(null).
        if (!closed) {
            closed = true;
            persistState();
        }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        // Emoji autocomplete (not in the command console: console input is commands only)
        if (!COMMAND_CONVERSATION_ID.equals(currentConversation)
        ...
        if (currentType == ChatMessageData.ConversationType.COMMAND) {
            ...
            if (history.isServerConnected()) {
                this.minecraft.player.connection.getConnection().send(
                    new net.minecraft.network.protocol.game.ServerboundCustomPayloadPacket(ServerboundCommandMessagePayload.ID, new ServerboundCommandMessagePayload(
                                    Component.Serializer.toJson(Component.literal(stripped)),
                                    this.minecraft.player.getUUID()).toBuf()));
            }
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        // Command component hover tooltips
        if (COMMAND_CONVERSATION_ID.equals(currentConversation)) {
            synchronized (cmdHitBoxes) {
                for (CommandHit hit : cmdHitBoxes) {
                    if (mouseX >= hit.x && mouseX <= hit.x + hit.w && mouseY >= hit.y && mouseY <= hit.y + hit.h) {
        ...
        // Rich-text link tooltips (hidden while an overlay panel is open)
        ...
        // Overlay panels (emoji / item picker / menus) render on top of everything,
        // so message items and tooltips can never cover them.
    ...
    private void renderTooltips(GuiGraphics g, int mouseX, int mouseY, int screenWidth, int screenHeight) {
        ...
        // Item tooltip, hidden while a context menu or an overlay panel is open
    ...
    private void drawWidgetPanels(GuiGraphics g, int mouseX, int mouseY) {
        // Render above message item icons (z=150) and below tooltips (z=400),
        // same depth as the context menus.
        g.pose().pushPose();
        ...
        if (contextType == CTX_AVATAR) {
            ...
            if (mx >= menuX && mx <= menuX + menuW && my >= menuY && my <= menuY + menuH) {
                if (my <= menuY + 20) {
                    ...
                    // Private message: switch to (or open) the DM conversation (self-DM allowed)
    ...
    private void buildSidebarEntries() {
        ...
        // In stream mode the private list and the console page are mutually exclusive;
        // opening the private list (rail avatar) hides the console entry.
    ...
    private void drawRail(GuiGraphics g, int mouseX, int mouseY, long now) {
        ...
        for (int i = 0; i < railItems.size(); i++) {
            ...
            // Stream pill (fully rounded, lifts on hover)
            int drawY = y;
            if (over) drawY -= 2;
            // Pixel-style corners
            int r = 8;
            ...
            switch (item.kind) {
                case RK_GROUP -> {
                    if (item.channelId != null) {
                        ...
                        // Unread dot for this group (or subchannel)
                ...
                case RK_CONSOLE -> {
                    ...
                    // Unread dot for the console conversation
    ...
    private void drawSidebar(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < sidebarEntries.size(); i++) {
            ...
            if (entry.isHeader) {
                if (Theme.stream()) {
                    // Category caption (muted uppercase)
    ...
    private void renderMessages(GuiGraphics guiGraphics, int mouseX, int mouseY, int screenWidth, int screenHeight) {
        ...
        for (int i = idx; i >= 0; i--) {
            ...
            // Stream unread divider, above the oldest unread message
            ...
            if (streamRows && msg.conversationType() != ChatMessageData.ConversationType.COMMAND) {
                // Merge into the older row (Discord-style)
    ...
    private RowPaint renderMessageRow(GuiGraphics g, ChatMessageData msg, int areaLeft, int areaRight, int y,
        ...
        // Timestamp right-aligned in the header; dup label left of it
        if (!merge) {
            ...
            // Merged rows keep their dup label at the line end
            String dupLabel = "x" + msg.duplicateCount();
        ...
        if (pureEmoji) {
            ...
            for (int li = 0; li < displayLines.size(); li++) {
                ...
                if (hasClickable(line)) {
                    synchronized (richTextHitBoxes) {
                        richTextHitBoxes.add(new RichTextHit(textX, textY + li * lineH + emojiOff,
    ...
    private void sendChannelPacket(ServerboundChannelActionPayload.Action action, String channelId, UUID ownerUuid) {
        if (this.minecraft == null || this.minecraft.getConnection() == null) return;
        var conn = this.minecraft.getConnection().getConnection();
        conn.send(new net.minecraft.network.protocol.game.ServerboundCustomPayloadPacket(ServerboundChannelActionPayload.ID, new ServerboundChannelActionPayload(action, channelId, ownerUuid,
