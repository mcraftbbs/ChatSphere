...
public class ConfigScreen extends Screen {
    ...
    private void buildCats() {
        ...
        behavior.add(new Opt("config.chatsphere.scroll_history_limit",
            y -> mkIntBox(y, String.valueOf(ModClientConfig.CONFIG.scrollHistoryLimit.get()), 50, 1000, 3, v -> { ModClientConfig.CONFIG.scrollHistoryLimit.set(v); scheduleConfigSave(); }), null));
        behavior.add(new Opt("config.chatsphere.command_history_limit",
            y -> mkIntBox(y, String.valueOf(ModClientConfig.CONFIG.commandHistoryLimit.get()), 10, 500, 3, v -> { ModClientConfig.CONFIG.commandHistoryLimit.set(v); scheduleConfigSave(); }), null));
        behavior.add(new Opt("config.chatsphere.url_filter", y -> mkStrBox(y, String.join(", ", ModClientConfig.CONFIG.urlLinkFilter.get()), v -> {
    ...
    private void drawStreamPreview(GuiGraphics g, int x, int y, int w, int h, double s) {
        int railW = (int) (13 * s);
        g.fill(x, y, x + railW, y + h, Theme.railBg());
        for (int i = 0; i < 3; i++) {
            int iy = y + (int) (7 * s) + i * (int) (10 * s);
            int is = (int) (6 * s);
            g.fill(x + (railW - is) / 2, iy, x + (railW + is) / 2, iy + is, i == 0 ? Theme.railIconGlyph() : Theme.railIconGlyphDim());
        }
    ...
    private void sendConfigUpdate(String key, String value) {
        ...
        mc.getConnection().getConnection().send(
            new net.minecraft.network.protocol.game.ServerboundCustomPayloadPacket(ServerboundConfigUpdatePayload.ID, new ServerboundConfigUpdatePayload(key, value).toBuf()));;
    }
    ...
    private static boolean safeGetBool(CfgValue.Bool cfg) {
        try {
            return cfg.get();
    ...
    private static String safeGetStr(CfgValue.Str cfg, String def) {
        try {
            Object val = cfg.get();
    ...
    private static String safeGetStr(CfgValue.Int cfg, String def) {
        try {
            return String.valueOf(cfg.get());
        } catch (IllegalStateException e) {
            return def;
        }
    }
    ...
    private void tryOpenServerConfig() {
        ...
        if (mc.getConnection() != null) {
            mc.getConnection().getConnection().send(
                new net.minecraft.network.protocol.game.ServerboundCustomPayloadPacket(ServerboundPermissionCheckPayload.ID, new ServerboundPermissionCheckPayload("SERVER_CONFIG").toBuf()));;
            ...
            if (isCornerStyleCat(selectedCat)) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if (mouseX >= cx2 && mouseX <= cx2 + cardW && mouseY >= cardY && mouseY <= cardY + cardH) {
                        ...
                        String curFile = CustomTheme.INSTANCE.currentFile();
                        if (CustomTheme.INSTANCE.isActive() && !CustomTheme.isPreset(curFile)) {
                            // custom theme: only switch its corner style
    ...
    /** Debounce per-keystroke saves. */
    private void scheduleConfigSave() {
        scheduledSaveAt = System.currentTimeMillis() + 400;
    }

    @Override
    public void tick() {
        super.tick();
        if (scheduledSaveAt > 0 && System.currentTimeMillis() >= scheduledSaveAt) {
            scheduledSaveAt = -1;
            CONFIG_SPEC.save();
        }
    }
